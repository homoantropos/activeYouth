...
export class AppointmentPlaceListComponent implements OnInit {
  ...
  get appointmentPlaces(): Array<AppointmentPlace> {
    return AppointmentPlaceListComponent.appointmentPlaces;
  }
  ...
  ngOnInit(): any {
    this.appointmentPlaceService.getAllAppointmentPlaces().subscribe(
      appointmentPlaces => {
        AppointmentPlaceListComponent.appointmentPlaces = appointmentPlaces.slice();
      }
    );
  }
        ...
        .subscribe(
          response => {
            ...
            AppointmentPlaceListComponent.appointmentPlaces =
              AppointmentPlaceListComponent.appointmentPlaces.filter(ap => ap.id !== this.appointmentPlaceId);
          },
          error => {
